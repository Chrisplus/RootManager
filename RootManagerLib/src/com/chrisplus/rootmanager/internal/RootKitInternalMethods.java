...
public final class RootKitInternalMethods {
  ...
  public static void getInstance() {
    // this will allow RootKit to be the only one to get an instance of this class.
   ...
   * if (remountAsRw) {
   * RootKit.remount(destination, "RW");
   * }
   * // if cp is available and has appropriate permissions
   * if (checkUtil("cp")) {
   * RootKitController.log("cp command is available!");
   ...
   * RootKitController.log("busybox cp command is available!");
   * if (preserveFileAttributes) {
   ...
   * if (checkUtil("cat")) {
   * RootKitController.log("cp is not available, use cat!");
   ...
   * if (remountAsRw) {
   * RootKit.remount(destination, "RO");
   * }
   * } catch (Exception e) {
   * e.printStackTrace();
   * result = false;
      ...
      Command command = new Command(RootKitConstants.GI, "/data/local/ls -i " + file) {
        ...
        public void output(int id, String line) {
          if (id == RootKitConstants.GI) {
            if (!line.trim().equals("")
                && Character.isDigit((char) line.trim().substring(0, 1).toCharArray()[0])) {
   ...
   * {
   * RootKitController.log("Killing process " + processName);
   ...
   * setData(pid);
   * }
   * RootKitController.log("Found pid: " + pid);
   * }
   * else
   * {
   * RootKitController.log("Matching in ps command failed!");
   * }
   * }
   * catch (Exception e)
   * {
   * RootKitController.log("Error with regex!");
   * e.printStackTrace();
   * }
   * }
   * }
